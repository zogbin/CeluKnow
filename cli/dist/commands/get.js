    ...
    try {
            ...
            if (opts.related) {
                ...
                if (doc.category_ids) {
                    ...
                    for (const catId of catIds) {
                        const catDocs = await api.getCategoryDocuments(catId);
                        output.same_category.push(...catDocs.filter((d) => d.id !== doc.id));
                    }
                }
            }
        ...
        if (opts.related) {
            ...
            if (doc.category_ids) {
                ...
                for (const catId of catIds) {
                    const catDocs = await api.getCategoryDocuments(catId);
                    const others = catDocs.filter((d) => d.id !== doc.id);
                    if (others.length > 0) {
                        ...
                        if (others.length > 10)
                            console.log(chalk.gray(`    ... 共 ${others.length} 篇`));
                        console.log();
                    }
                }
            }
